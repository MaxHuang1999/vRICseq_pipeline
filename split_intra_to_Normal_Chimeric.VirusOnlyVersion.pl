#!/usr/bin/perl
die "perl $0 pcp_rep1_interaction.without_rRNA.sam gapped_reads_overlapped_with_exon_junction.list pairreads_distance_in_virus minimal_fragment_len_for_pair (must be large than num used in creat_gapped_and_Align_pair_bed.pl)\n" if(@ARGV != 4);
my $in_sam=shift;
my $junction_reads=shift;
my $pairreads_distance_in_virus=shift;
...
open(PDV,$pairreads_distance_in_virus) || die;
while(my $line=<PDV>){
	chomp $line;
	my @sub=split/\s+/,$line;
...
open(OUTM,">junction.sam") || die;
open(OUTH,">element.sam") || die;
					...
					if($strand_a eq "Plus"){
						...
						if($fragment_len < 0){	#conflict direction
							...
							if(exists $close_reads_in_mature{$sub_a[0]}){
								...
								print OUTD $frag_a,$frag_b;
								#since we did not require strand for classify interGene and intraGene
                print OUTM  $frag_a,$frag_b;
                print OUTH  $frag_a,$frag_b;            
								print LOG $sub_a[0],"\tFragmentLongerThan$minimal_fragment_len_for_pair","inPre-RNAButNoAvailableTranscripts\tDropped\n";
							}
						}
					}
